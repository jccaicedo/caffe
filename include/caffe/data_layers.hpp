...
/**
 * @brief Provides data to the Net from images directly without using leveldb
 *        data structures.
 */

template <typename Dtype>
class NoLevelDBDataLayer : public BasePrefetchingDataLayer<Dtype> {
 public:
  ...
  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_NOLEVELDB;
  }
  virtual inline int ExactNumBottomBlobs() const { return 0; }
  virtual inline int ExactNumTopBlobs() const { return 2; }
 ...
 protected:
  ...
  shared_ptr<Caffe::RNG> prefetch_rng_;
  vector<std::pair<std::string, int> > image_database_;
};

/**
 * @brief Provides data to the Net from a file prepared for Q Learning
 */

template <typename Dtype>
class QDataLayer : public BasePrefetchingDataLayer<Dtype> {
 ...
 public:
  ...
  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_QDATA;
  }
  virtual inline int ExactNumBottomBlobs() const { return 0; }
 ...
 protected:
  ...
  shared_ptr<Caffe::RNG> prefetch_rng_;
  vector<std::pair<std::string, vector<int> > > image_database_;
  Blob<Dtype> prefetch_state_features_;
  enum QField { IMAGE_INDEX, ACTION, REWARD, DISCOUNTEDMAXQ, X1, Y1, X2, Y2, PREV_ACTION, NUM };
  vector<vector<float> > windows_;
};

/**
 * @brief Provides data to the Net from a text file with explicit features
 */

template <typename Dtype>
class PlainDataLayer : public BasePrefetchingDataLayer<Dtype> {
 ...
 public:
  ...
  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_PLAIN_DATA;
  }
  virtual inline int ExactNumBottomBlobs() const { return 0; }
  virtual inline int ExactNumTopBlobs() const { return 2; }
